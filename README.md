...
**55 типов**: 6 встроенных (`src/types/documents.ts:19`) + 49 каталожных
...
| `cargo_damage_act` | Акт о неисправностях груза (CDS) | act_number, act_date, awb_ref, damages |
...
| `info_letter` | Информационное письмо | subject, sender, letter_date |
...
| `product_description` | Техническое описание товара | product_name, model, purpose, dimensions |
...
- **55 типов документов** в каталоге: 6 встроенных + 49 каталожных
  (проверено по `migrations/*.sql` и живому каталогу стенда).
...
| Дата | Что |
|---|---|
| 2026-07-22 | Релиз `v0.2.0` |
| 2026-07-21 | +3 типа по аудиту пустых документов: `cargo_damage_act`, `product_description`, `info_letter` |
| 2026-07-21 | Устойчивость очереди: COMPLEX-DEFER (тяжёлый документ откладывается), stall-guard, снятие terminal-дубликата при reprocess |
| 2026-07-21 | Ускорение извлечения SPEED-1/2/5: нарезка по строкам таблицы, дедуп xlsx-повторов, эскалация обрыва |

...
| **WebSocket / SSE / Upgrade-headers** | включены в nginx как страховка; сервис использует чистый REST + polling |
| **Внешний порт** | host `8085` → `doc-service:3000` (UI + API) |
| **nginx-параметры** | `client_max_body_size 50m`, `proxy_read_timeout 600s` |
...
Сетевая обвязка:
  - Поддомен: parsedocs.taipit.ru
  - TLS через корп. nginx, HTTPS-only
  - Host-port: 8085 → контейнер 3000
  - nginx-параметры: client_max_body_size 50m, proxy_read_timeout 600s,
    WS-headers (proxy_http_version 1.1, Upgrade $http_upgrade, Connection $connection_upgrade)
...
Что готово:
  ...
  - OWNERS.md (этот файл)
  - secret-keys (.env реальные) готовы, передаются отдельно

Чего прошу:
  1. proxy_pass parsedocs.taipit.ru → 10.10.13.10:8085 + TLS
  2. Открыть порт 8085 в firewall сервера
  (Compose, репо, секреты — разворачиваем сами)

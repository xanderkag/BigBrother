...
>
> **Последнее обновление:** 2026-06-02 (bench v3 + кросс-валидация, прогоны #29–35)

---

## TL;DR — что выбрали и почему

> **Сменился дефолт (bench v3, июнь):** появился сервер на 96 ГБ VRAM
> (`10.10.33.10`) — прежний потолок 20 ГБ снят. Phi-4 был дефолтом не
> потому что лучший, а потому что влезал. На реальном golden-set
> победитель — **Mistral Small 3.1 24B**: догоняет 70B по точности при
> ~15 ГБ и 5× скорости. Детали — MODEL_REPORT #29–35.

| Слот | Модель | Обоснование |
|---|---|---|
| 🥇 **Локальный production-дефолт** | **Mistral Small 3.1 24B** (text, после OCR) | поля **98.3%**, `total`-арифметика **100%**, медиана **~5 с/файл**, ~15 GB VRAM. Лучший баланс точность/скорость/VRAM. Кросс-валидирован (#35, без оверфита) |
| 🥈 **Максимум точности** | Llama 3.3 70B / Qwen2.5 72B (text) | те же 98.3% / 100%, но ~43 GB и медленнее — запас под сложные типы, если есть VRAM |
| 💰 **Дешёвый / роутер** | Gemma 3 27B (text) | поля 96.7%, арифметика 85.7% при ~17 GB, медиана ~6 с — для co-residence (простые типы) |
...
| 🖥 **GPU-стенд** | bench v3: 96 GB VRAM, узел `10.10.33.10` · bench v2 (исторический): RTX 4000 Ada 20 GB, `10.10.28.10` | На 96 ГБ влезают и 70B; на 20 ГБ всё >32B Q4 уходило в CPU offload |

**Главный вывод:** для документов с текстовым слоем (PDF, хороший скан) **text-пайплайн (OCR → Mistral Small 3.1 24B) стабильно бьёт vision** по арифметике сумм. Vision держим для фото/кривых сканов.

---

## Сводная таблица — bench v3 (9 doc real golden-set, текст-слой, 96 ГБ)

Главная цель проекта — `total`-арифметика ≥85%. Полные данные: [`MODEL_REPORT.md`](./MODEL_REPORT.md) #29–35.

| Модель | Поля | `total` | Время/файл | VRAM | Вердикт |
|---|---|---|---|---|---|
| **Mistral Small 3.1 24B** 🏆 | **98.3%** | **100%** | 3–26 с (med ~5) | ~15 GB | дефолт — точность 70B при 5× скорости |
| Llama 3.3 70B | 98.3% | 100% | 11–89 с | ~43 GB | эталон точности, дорогой по VRAM |
| Qwen2.5 72B | 98.3% | 100% | 11–119 с | ~43 GB | ничья с Llama (мультиязык/таблицы) |
| Gemma 3 27B | 96.7% | 85.7% | 4–40 с (med ~6) | ~17 GB | берёт цель дёшево — кандидат в роутер |
| DeepSeek-R1 70B | 96.7% | 85.7% | 10–88 с | ~43 GB | reasoning, чуть ниже топа |
| Phi-4 14B (baseline) | 88.3% | 71.4% | 4–32 с | ~9 GB | прежний дефолт — цель не берёт |

---

## Сводная таблица — bench v2 (10 PDF + ground-truth) — *исторический, эпоха 20 ГБ*
...
| **#28** | **Qwen2.5-VL 7B vs 32B head-to-head** (точность + латентность) | последний прогон эпохи 20 ГБ |

### Phase 5 — bench v3 на 96 ГБ VRAM (9 doc golden-set, июнь 2) — *снят потолок VRAM*
Узел `10.10.33.10` (96 ГБ): впервые влезают 70B-модели целиком. Цель — `total`-арифметика ≥85%.
| # | Модель | Итог |
|---|---|---|
| #29 | Phi-4 14B (baseline на новом стенде) | поля 88.3%, total 71.4% — цель не берёт, прежний дефолт сместили |
| #30 | Llama 3.3 70B (text) | поля 98.3%, total 100% — эталон точности, ~43 ГБ |
| #31 | DeepSeek-R1 70B (text) | поля 96.7%, total 85.7% — reasoning, чуть ниже топа |
| #32 | Qwen2.5 72B (text) | поля 98.3%, total 100% — ничья с Llama, ~43 ГБ |
| #33 | Gemma 3 27B (text) | поля 96.7%, total 85.7% при ~17 ГБ — кандидат в роутер |
| **#34** 🏆 | **Mistral Small 3.1 24B (text)** | **поля 98.3%, total 100%, ~5 с/файл, ~15 ГБ — новый дефолт** |
| #35 | Mistral Small 3.1 24B — кросс-валидация на corpus-gt | подтверждение #34 без оверфита |
...
| Отчёт | Что внутри |
|---|---|
| [`doc-service/docs/archive/REAL_DOCS_FULL_REPORT_2026-05-18.md`](../doc-service/docs/archive/REAL_DOCS_FULL_REPORT_2026-05-18.md) | Полный прогон на VED-документах EWL/ANJI MINGPAI: PDF-text ✅, scan ⚠️, XLSX ✅, EAC-cert 10MB. Выявленные баги (classifier priority, single-shot OOM на длинных текстах), production readiness *(архив 02.06)* |
| [`doc-service/docs/archive/REAL_DOCS_SMOKE_RESULTS_2026-05-18.md`](../doc-service/docs/archive/REAL_DOCS_SMOKE_RESULTS_2026-05-18.md) | Smoke на 3 реальных: Акт взвешивания, Коносамент FESCO, ВТБ заявление на перевод. Что работает / что подкрутить *(архив 02.06)* |
...
GPU-стенд (bench v3, актуальный): узел `10.10.33.10` — 96 ГБ VRAM, ollama на `:11434`, влезают 70B-модели. Исторический (bench v2): `10.10.28.10` (RTX 4000 Ada, 20 GB). Оба — у владельцев узлов (не наш прод `10.10.13.10`).

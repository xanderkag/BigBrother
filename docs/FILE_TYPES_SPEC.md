...
1. **DOCX поддержка**: +600 МБ к image (LibreOffice) или sidecar-контейнер
   или вообще отказывать? Если поток DOCX будет ≤5% — отказ + инструкция
   «сохраните как PDF» приемлемо.

2. **Лимит на распакованный архив**: 500 МБ хватит на нормальные кейсы
   (пачка счетов за месяц), bomb-protected. Согласовать с бизнесом.

3. **Email retention**: храним ли исходные `.eml`/`.msg` после распаковки?
   Если да — где (volume parent_files), на сколько (30 дней).

4. **Multi-doc auto-split поведение**: автоматически разделять «склейку»
   или загрузка как один документ + кнопка «разделить» в UI оператору?
   Авто-сплит экономит время, но риск ложно-разделить большой документ.

5. **Pre-processing для уже-обработанных job'ов**: новые форматы появятся
   когда уже есть N jobs. Делать ли migration / reprocess для старых?
   Я бы не делал — оператор перезагрузит если нужно.

6. **EXIF stripping**: фото со смартфона содержит GPS координаты, имя
   автора, серийник камеры. По 152-ФЗ это персональные данные. Зачищать
   EXIF перед сохранением оригинала? Делать ли это опционально через flag?

7. **Test corpus в репо**: куда складывать большие тестовые файлы?
   Git-LFS? Отдельный S3 bucket с pull-скриптом? Или генерить синтетически?

...
- **VL-FINETUNE-PILOT (2026-07-21) — ЭПИК, до востребования: пилот дообучения Qwen3-VL-8B под наши сканы.**
  ТЗ от DevOps/GPU-инфры: `Desktop/ТЗ_Дообучение-VL-пилот_2026-07-21.md` (узкий пилот: 1 тип, LoRA/QLoRA на Blackwell 33.10, замер «до/после», ship только при выигрыше над базовой 32B). План изучен и спланирован 2026-07-21, **в работу НЕ запущен** — ждёт команды владельца.

  **Ответы на §3 ТЗ (сняты с прод-БД 2026-07-21, скан-путь = tesseract/vision):**
  - Боль+история: **`commercial_invoice` — рекомендованный пилотный тип** (132 скана, 110 needs_review = 83%, многотоварные таблицы, билингв кит/англ/рус). Альтернативы: transport_invoice (35/74%), smgs (28/82%) — боль есть, истории мало. bill_of_lading (342 скана) НЕ кандидат: 3% review, база справляется, дообучать некуда.
  - §3.3 (ключевое): **валидированных извлечений почти нет** — `extracted_corrected_at` = 1 док на всю базу, `job_feedback` = 0. У нас только сырой вывод модели + статусы done/needs_review. «Золото» надо СОЗДАВАТЬ — это главная работа пилота, не обучение.

  **План (5 этапов):**
  1. **Фабрика меток**: high-conf done → авто-метка; needs_review → ручная правка оператором в существующем UI (механизм `extracted_corrected_at` готов, им не пользовались). Первый срез ~100 проверенных + 30 отложенных на тест. ⚠️ авто-метки high-conf сначала проверить выборкой 30-50 шт (риск «учим на своих ошибках», §2 ТЗ).
  2. **Экспорт-скрипт** images+JSONL по формату §4 ТЗ (файлы в storage, промпт/схема из резолвера, confidence, период) — ~1 день.
  3. **ПДн/конфиденциальность (§6)**: скраб паспортных паттернов на выгрузке + обезличенные doc_id; маскировать ФИО в картинках нельзя (портит обучение) → решение владельца: обучение только в контуре 33.10, датасет не покидает бокс. Связано с открытым решением «ПДн в raw_text обычных доков» (вариант А/Б/В — тоже ждёт владельца).
  4. Отложенный тест + наш eval-harness поле-в-поле (есть с БКТ) — DevOps меряет базу (§7.1).
  5. **Гейт**: ship только если дообученная 8B бьёт базовую 32B на проблемных полях (items[], HS-коды, суммы).

  **Первый шаг при запуске:** оформить ответ DevOps (§3-ответы + выбор типа + условия §6) и завести операторский процесс разметки.
...
- **Audit follow-ups (2026-05-27) — задокументированы, не фикшены в этом заходе.** H1 (startup cross-validation byo+no-key / asr+no-url), M1 (0..1 bounds на confidence-knob'ах), L1 (capability-флаги в `/capabilities`) — закрыты. Остаток:
  ...
  - ~~**M3 — forced-provider fallthrough без observability.**~~ ✅ РЕШЕНО 2026-07-01. При плохом `_force_provider_id` роутинг по-прежнему молча откатывается на default (поведение НЕ тронуто), но теперь наблюдаемо. `DynamicLlmClient.probeForceProvider(id)` — observability-only проба (без side-effect, без создания клиента), зеркалит ветки `resolveById` и возвращает reason: `not_found` | `non_llm_kind` | `missing_base_url` | `lookup_error` (или null если резолв ОК; `stub` → рабочий Null-клиент, НЕ fallthrough). `orchestrator.processJob` вызывает пробу перед `withForceProvider`: при отказе пишет `log.warn({jobId, force_provider, reason}, 'forced LLM provider did not resolve; falling through to default provider')` (id не секрет; base_url/ключи НЕ логируются) + инкрементит counter `docservice_extractor_forced_provider_fallthrough_total{reason}` (id НЕ лейбл — unbounded cardinality). Routing-исход байт-идентичен (пробы результат не используется в решении — реальный fallthrough как и раньше делает `delegate()`). Тесты `doc-service/tests/forced-provider-fallthrough.spec.ts` (9): все 4 reason'а + happy/stub + orchestrator-блок (warn+counter, withForceProvider всё равно выполняется).

...
/**
 * Регрессия набора из 15 боевых документов (2026-07-24). В спецификации
 * «Товар в грузе» позиции лежали двумя блоками ОДНОГО листа — 9 и 6 строк,
 * оба по три колонки: одна таблица, разорванная строкой-разделителем. Модель
 * вернула только первый блок, и вышло 9 позиций вместо 15. Сторож «пропущена
 * область крупнее выбранной» этого не ловит: 6 меньше 9.
 */
describe('разорванная таблица на одном листе → откат, а не половина позиций', () => {
  const SPLIT: OcrTable[] = [
    {
      sheet: 'Лист1',
      rows: [
        ['Артикул', 'Товары (работы, услуги)', 'Количество'],
        ...Array.from({ length: 9 }, (_, i) => [`61256${88 + i}`, `Компьютерный корпус PS20${i}`, '300']),
        ['', '', ''],
        ['Артикул', 'Товары (работы, услуги)', 'Количество'],
        ...Array.from({ length: 6 }, (_, i) => [`71256${88 + i}`, `Блок питания PM-30${i}`, '150']),
      ],
    },
  ];
  const TEXT = ['Артикул,Товары,Количество']
    .concat(Array.from({ length: 15 }, (_, i) => `код${i},товар ${i},100`))
    .join('\n');

  it('невыбранный блок той же ширины на том же листе → быстрый путь уступает', async () => {
    const calls: Call[] = [];
    const parser = new MultiPassLlmParser(
      makeLlm({
        mapping: {
    ...
    const res = await parser.parse(TEXT, { llmSchema: SCHEMA, tables: SPLIT });

    const issues = (res.extracted._issues ?? []) as string[];
    expect(issues.some((i) => i.startsWith('xlsx_fast_split_table:Лист1'))).toBe(true);
    // Ушли прежним путём — половины позиций клиент не увидит.
    expect(calls.filter((c) => c.kind === 'chunk').length).toBeGreaterThan(0);
  });
});

/**
 * Регрессия набора из 15 боевых документов (2026-07-24). В манифесте
 * авиаотправок таблица на 10 колонок, а модель разметила две. Строк вышло
 * даже больше прежнего (120 против 99), но каждая почти пустая — прежний путь
 * заполнял 5.5 поля на позицию. По числу позиций это выглядело выигрышем.
 */
describe('разметка должна покрывать таблицу, а не уголок её', () => {
  const WIDE: OcrTable[] = [
    {
      sheet: 'Schedule',
      rows: [
        ['SHIP TO', 'ETC', 'Flight', 'Airport', 'Invoice No.', 'Description', 'B/L No.', 'Forwarder'],
        ...Array.from({ length: 20 }, (_, i) => [
          'Russia', `2${i}-Apr-23`, `CI70${i}`, 'SVO', `23302${i}`, `груз ${i}`, `BL${i}`, 'EWL',
        ]),
      ],
    },
  ];
  const TEXT = ['SHIP TO,ETC,Flight,Airport,Invoice No.,Description,B/L No.,Forwarder']
    .concat(Array.from({ length: 20 }, (_, i) => `Russia,2${i}-Apr-23,CI70${i},SVO,23302${i},груз ${i},BL${i},EWL`))
    .join('\n');

  it('размечено 2 колонки из 8 → откат, позиции не должны быть пустыми', async () => {
    const calls: Call[] = [];
    const parser = new MultiPassLlmParser(
      makeLlm({
        mapping: {
          regions: [
            // Оба поля размечены ВЕРНО (наименование и числовой номер инвойса) —
            // проверка типов пройдёт, отказать должен именно сторож покрытия.
            { region: 0, mapping: [{ field: 'name', column: 5 }, { field: 'quantity', column: 4 }] },
          ],
        },
        calls,
      }),
    ...
    const res = await parser.parse(TEXT, { llmSchema: SCHEMA, tables: WIDE });

    const issues = (res.extracted._issues ?? []) as string[];
    expect(issues.some((i) => i.startsWith('xlsx_fast_thin_mapping:Schedule:2/8'))).toBe(true);
    expect(calls.filter((c) => c.kind === 'chunk').length).toBeGreaterThan(0);
  });
});

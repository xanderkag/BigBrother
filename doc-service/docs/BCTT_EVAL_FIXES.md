...
## ⚠️ Приёмка на asha (2026-07-14, `86482fe` задеплоен, миграция `ocr_pages` применена)
Прогон с флагами ON (`VLM_CLASSIFY=true`, `MULTIDOC_LLM_CLASSIFY=true`), `_skip_cache`:
- ✅ **FIX-3 РАБОТАЕТ:** viber_259 → `contract_specification` (был `commercial_invoice`). Без флагов.
- ✅ **Cost-фича РАБОТАЕТ:** `cost_rub` считается (SICHEL 24.87₽, viber_259 1.34₽, pac_2 4.63₽), `cost_estimate=false`.
- ⚠️ **FIX-4 частично:** pac_2 → `invoice` (не `price_list`, но и НЕ `packing_list`). Headerless-страница, ожидаемо.
- ❌ **FIX-1 НЕ сработал:** SICHEL СТС (стр.15) **остался склеен** с паспортом → сегмент `driver_passport(14-15)`,
  отдельного `vehicle_registration` НЕТ. **Гипотеза причины:** триггер VLM = «текст страницы скуден < `vlmMinText`(120)»,
  а эстонская СТС-страница даёт >120 симв. частичного/мусорного OCR → НЕ «scant» → `classifyPageImage` не зовётся →
  страница приклеивается к предыдущему сегменту. **Нужно агенту:** триггерить VLM когда per-page **keyword дал
  null/low-conf** (а не только по длине текста), либо поднять порог; проверить на SARASA/KARINA (СТС после CMR, без паспорта).
- ⚠️ **FIX-2/флаги дорого:** `MULTIDOC_LLM_CLASSIFY=true` переразбил 4-стр. инвойс SICHEL на 4 сегмента
  (правило «тот же № инвойса → тот же сегмент» не держит при per-page LLM), и **флаги задрали цену 5×**
  (SICHEL 25₽ против ~5₽). → **Флаги откачены в `false`** (owner чувствителен к цене; FIX-1 всё равно не чинит цель).
- **Итог на asha сейчас:** FIX-3 + FIX-4(слабый) + cost-фича активны (без флагов). FIX-1/FIX-2 — на доработку
  (триггер VLM + удержание сегмента инвойса + стоимость). До рабочего FIX-1 СТС в композитах теряется — известный пробел.

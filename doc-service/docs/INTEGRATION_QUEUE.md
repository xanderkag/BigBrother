...
### Q-NEWTYPES-3. Два новых типа (2026-07-16) + сверка полного расширенного каталога (51 тип)

- **Status:** `OPEN` (To: SLAI_DEV — heads-up, **НЕ блокер**: приём v1 устойчив к незнакомым типам по Q-CLSF-ONTO-1; просим добавить рус. ярлыки + галочку «не ломается»)
- **Asked:** 2026-07-16
- **From:** PARSDOCS_DEV → SLAI_DEV (нота `SLAI_NEWTYPES_2026-07-16.md`)
- **Что нужно:** SLAI знает про 2 новых slug'а, добавляет ярлыки в свой словарь, подтверждает устойчивость приёма.
- **Что сделать когда ответят:** ничего в коде (типы в проде, аддитивно) → RESOLVED.

#### Context / что нового
По анализу боевого прогона 2026-07-16 заведены 2 типа (миграция `20260716000001`,
`tier=beta`, в проде, оба уже ловятся вживую — `document_request` +1, `empty_container_return` +1 за сутки):
- **`empty_container_return`** — «Инструкция по возврату порожнего контейнера» (терминал/депо возврата, срок, номера порожних, линия). НЕ транспортная накладная, НЕ booking.
- **`document_request`** — «Запрос документов» (перечень запрашиваемых, срок, ссылка на заказ).

Контракт: **новые значения строки `document_type`**, аддитивно — `schema_version`
остаётся **1.4** (прецедент 2026-07-02: новые типы версию не бампают; бампаем только
на структурные изменения payload). Envelope `version` не трогается. По Q-CLSF-ONTO-1:
slug'и стабильны, SLAI устойчив к незнакомым типам (карточка «как есть»).

#### Полный расширенный каталог на 2026-07-16 — 51 активный тип
Свежий хвост с прошлой полной сверки (для словаря ярлыков SLAI):
- **6 ВЭД-типов** (GO SLAI 2026-07-12, в проде с 2026-07-15): `excise_ead`, `vehicle_registration`, `driver_passport`, `transport_permit`, `certificate_register`, `delivery_note`.
- **2 новых** (2026-07-16): `empty_container_return`, `document_request`.
Остальные 43 без изменений. Полный список 51 slug — в `SLAI_NEWTYPES_2026-07-16.md`.
...
### Q-DEEPPASS-1. Глубокий разбор нераспознанного остатка (deep-pass) + контракт `_deep`

- **Status:** `RESOLVED` 2026-07-15 (SLAI подтвердил совместимость 1.4 + принял `_deep` у себя; `cargo_photo` — их предложение, заведено). Осталось ops: включить флаг `DEEP_PASS_ENABLED` на боксе (owner/parallel lane).
- **Asked:** 2026-07-15
- **From:** PARSDOCS_DEV → SLAI_DEV (бриф `SLAI_DEEP_PASS_BRIEF_2026-07-15.md`)
- **Что нужно:** SLAI подтверждает приём `schema_version 1.4` + нового ключа `extracted._deep`; опц. показать категорию/резюме у себя.

#### Что сделано (parsdocs)
Второй ярус для остатка, который рабочий классификатор не опознал (или OCR-refusal):
локальная модель читает документ (текст ≥300 симв. или картинка 1-й стр.), возвращает
**широкую категорию** (18 фикс. slug'ов) + **резюме** + **вердикт** (`mapped` /
`foreign_document` / `not_a_document`); при `mapped` — возврат в конвейер по схеме
рабочего типа (`method=deep_pass`, conf 0.7). След — в `extracted._deep` (у ВСЕХ
доков второго яруса, включая mapped). ПДн: `id_document` → резюме-константа, без
маппинга; всё локально. Попутно: детерминированные ошибки (OCR-refusal / неподдерж.
mime) больше не ретраятся 3× (`UnrecoverableError`).
Контракт: `schema_version` **1.3 → 1.4** (аддитивный `_deep`, minor по Q17).
UI: бейдж категории + разворот строки + карточка «Глубокий разбор».
Гейт `DEEP_PASS_ENABLED` (default OFF). Коммиты `f9a909c`, `61dfbfc`, `5699bd2`.

#### Answer (SLAI 2026-07-15)
1. **Совместимость 1.4 — подтверждена** (их гейт режет только major 2.x; minor 1.4
   логируется, `_deep` сохраняется как прочие служебные ключи). Закрыто тестом у них
   + проверено на живой базе.
2. **Приняли у себя:** `not_a_document` исключается из «Проверки» и очереди
   ассистента-доводчика (комплектность пакета не портится, из реестра не пропадает);
   для остальных неопознанных ассистент видит категорию+резюме. Бейдж категории в
   карточке — их следующий UI-шаг.
3. **Словарь:** предложили `cargo_photo` (фото погрузки контейнера — свидетельство
   отгрузки, отдельная плитка, не product_photo) + «шлите `_deep` у всех».

#### Resolution
`cargo_photo` добавлена (`5699bd2`, 18 категорий, зафиксирована в тесте). «`_deep` у
всех» — уже так (выставляется вне ветки `catalog_slug`), кода не потребовалось.
Осталось: включить `DEEP_PASS_ENABLED` на боксе (ops, не dev) — после включения
эффект виден сам собой (у unknown появляются описания, мусор/фото отсеиваются).

---

### Q-INGEST-1. Правила прогона документов из папок — сверка «нет разночтений»

- **Status:** `RESOLVED` 2026-07-15 (сверено по живому коду + данным стенда с обеих сторон; 5/5 совпало).
- **Asked:** 2026-07-15
- **From:** PARSDOCS_DEV → SLAI_DEV (сверка `SLAI_INGEST_RULES_SYNC_2026-07-15.md` → ответ `SLAI_ANSWERS_2026-07-15_INGEST.md`)
- **Что нужно:** зафиксировать одинаково понимаемые правила передачи документов из папок под заказ.

#### Итог сверки (5/5 совпало, разночтений нет)
1. **`order_hint` на каждом файле — да.** Их сканер ставит метку папки-заказа на
   каждый файл, включая вложенные подпапки (все наследуют папку заказа; проверено
   вживую: `262_00095\С отгрузки\...\1.jpg` → hint=`262_00095`). Мы эхаем `order_hint`
   top-level (одиночный) + per-segment (композит) — verified в коде (`deliver.ts:204`,
   `webhook-delivery.ts:131`). Единственный зазор: файл в **корне** обмена (не в папке)
   hint не получит — таких на практике нет.
2. **Папка = один заказ, 1:1.** Имя папки = внешний номер заказа, уникален.
3. **Отдельные файлы vs стопка — совпало.** Отдельные шлют по одному (мы не
   группируем), склеенный PDF отдают как есть (сегментируем сами).
4. **Дедуп — cache-hit по SHA-256 = ровно то, что нужно** (безопасная почасовая
   перезаливка). `force_reprocess` не нужен; у них поверх ещё схлопывание копий/версий.
5. **Канал:** для папок остаются на multipart (файлы уже на их сервере); `file_url`
   включим позже под крупные фрахт-сканы из чата.

Связь с Q-DEEPPASS-1: `not_a_document` из папок с 2026-07-15 исключается из «Проверки»
и очереди доводчика (фото/скриншоты больше не жгут GPU и не портят комплектность).
...
### Q-CLSF-ONTO-1. Классификатор v2: 6 новых ВЭД-типов + slug-согласование для v1-контракта

- **Status:** `RESOLVED` 2026-07-12 (SLAI GO — 6 типов заводим, фиче-флаг НЕ нужен)
- **Asked:** 2026-07-12
- **From:** PARSDOCS_DEV → SLAI_DEV
- **Что нужно:** подтверждение, что заводим 6 новых типов и что приём v1 не роняется на новых значениях `document_type`.
- **Что сделать когда ответят:** миграция типов (`tier=beta`) + при GO добавить в `OUTBOUND_SLUG_ALIASES`/фиче-флаг; иначе гейтить до согласования.

#### Question / Context
Ручной разбор всех 51 док. БКТ Транзит (`doc-service/docs/BCTT_GROUNDTRUTH.md`, 51/51 прочитано) выявил типы, которых нет в каталоге и которые встречаются в КАЖДОМ ВЭД-пакете. Полное ТЗ: `doc-service/docs/TZ_CLASSIFIER_PACKET_V2.md` §5, §13.
Новые типы: **`excise_ead`** (АКЦИЗЕ ПРЕЦЕС / e-AD Reg.684/2009 — в каждой алкогольной поставке, ОТДЕЛЬНО от обычного EAD), **`vehicle_registration`** (СТС/техталон), **`driver_passport`** (см. Q-CLSF-PII-1), **`transport_permit`** (дозвол/Engedély), **`certificate_register`** (реестр сертификатов ЕАС — таблица, не одиночный), **`delivery_note`** (расходная накладная).
Двойные документы в одном физлисте (§13.5): «Invoice Packing List» (Mondelez) и «СТС поверх дозвола» — предложение: один тип + `secondary_role`, не расщеплять физлист. Ок?
Контракт-риск: новые slug'и сейчас утекут в v1 дословно (`OUTBOUND_SLUG_ALIASES` их не содержит), в т.ч. в top-level `document_type` для композитов, где доминант — новый тип. Нужен ли фиче-флаг/алиасы до GO?
Целевой M1 приёмки (§13.8): ≥85% файлов с корректным набором типов по сегментам — ок?

#### Answer
SLAI 2026-07-12: **6 типов — да, заводим. M1 ≥85% ок** как ворота приёмки.
Новые slug'и **принимаем в v1 сразу, фиче-флаг НЕ нужен** — SLAI устойчив к
незнакомым типам (карточка «как есть», сущности не создаются, сверку не ломает).
Два обещания с нашей стороны: (1) slug'и **стабильны** после ввода (не
переименовывать); (2) изменения — **minor-бампом `schema_version`** (Q17: их
сторож ругается только на major). Русские ярлыки типов SLAI добавит у себя сам.
Двойные документы: **один тип + `secondary_role` — ок**, физлист не расщеплять;
`secondary_role` = slug из того же каталога.

#### Resolution
Миграция `20260711000001_ved_packet_types.sql` (commit `b3e0018`) завела 6 типов
`tier=beta`. Slug-gate в `slug-normalize.ts` **НЕ добавляем** (SLAI принимает
raw). Держим slug'и стабильными; при правках схем — minor-бамп `schema_version`.
`secondary_role` (Фаза 2, двойные доки) — значение из каталога.

---

### Q-CLSF-PII-1. Паспорта водителей — политика ПДн (prod-блокер композитов)

- **Status:** `RESOLVED` 2026-07-12 (SLAI: политика «классифицируем, не извлекаем» — ДА; `present` нужен)
- **Asked:** 2026-07-12
- **From:** PARSDOCS_DEV → SLAI_DEV
- **Что нужно:** подтверждение политики: `driver_passport` классифицируем, но персональные поля НЕ извлекаем (extract = `{doc_kind:"id", country, present}`).
- **Что сделать когда ответят:** —; §8 ТЗ (ПДн-гейт) — P0-блокер независимо, делаем ДО сегментации.

#### Question / Context
В шоферских папках БКТ паспорт водителя — сквозной элемент (3 грузовика = 3 паспорта; внутри композитов SICHEL/noreply и отдельными фото). **Пока композиты с паспортами гонять НЕЛЬЗЯ:** ревью ТЗ (сверено с кодом) нашло утечки ПДн — `jobs.raw_text` (`orchestrator.ts:732`) хранит OCR всех страниц вкл. паспорт и отдаётся наружу `GET /jobs/:id/raw_text` (`routes/jobs.ts:745`); `documents[]` обходит `redact_pii` (`webhook-delivery.ts:75-85`); паспортный текст уходит в облачный classify/extract (нарушение 152-ФЗ / правила asha «не корп-данные»). Фиксы — `TZ_CLASSIFIER_PACKET_V2.md` §8 (allowlist-фильтр `{doc_kind,country,present}`, маскирование raw_text ID-страниц, redact `documents[]`, не слать паспорт в облако, MRZ+иностранные коды в `pii-redact.ts`).
Вопрос к SLAI: политика «не извлекаем ПДн» — ок? Нужны ли вам ID-доки в потоке (флаг `present`) или полностью отбрасывать?

#### Answer
SLAI 2026-07-12: **политика ок и правильная.** `driver_passport` классифицируем,
персональные поля НЕ извлекаем, в облако не шлём, в `raw_text` маскируем. **Флаг
`present` НУЖЕН, не отбрасывать** — факт «в папке есть паспорт/СТС» участвует в
комплектности шофёрского пакета. Extract `{doc_kind:"id", country, present}` —
достаточно, больше ничего. Порядок «§8 ПДн → потом сегментация» поддерживают.
Заполнение карточек водителей из фото — отдельная история (VANGA-ID-1), только
локальной моделью и с решения владельца; сейчас не нужно.

#### Resolution
Фаза 1 (commit `b3e0018`) закрыла §8.2 (redact `documents[]`), §8.3 (allowlist
`{doc_kind,country,present}` — ровно запрошенное), §8.5a (MRZ + иностранные
коды в `pii-redact`). §8.1 (маскирование `raw_text` ID-страниц) и §8.5b
(не слать паспорт в облако) — Фаза 2 (нужен passport-детект сегментации).

---

### Q-CLSF-CONTRACT-1. Композит-контракт: `documents[]` vs parent-child + семантика `extracted`

- **Status:** `RESOLVED` 2026-07-12 (SLAI GO + 3 просьбы: per-segment status/confidence, order_hint на сегмент, segment_id)
- **Asked:** 2026-07-12
- **From:** PARSDOCS_DEV → SLAI_DEV
- **Что нужно:** решение по контракту для файлов-стопок (композит = один скан из 3–15 разных документов).
- **Что сделать когда ответят:** зафиксировать в `CONTRACT_TECH_APPENDIX` + eval-приёмку `scripts/eval-bctt.ts`.

#### Question / Context
~35/51 файлов БКТ — композиты (стопка EAD+акциз+инвойс+packing+CMR+СТС+паспорт в одном скане). Сегментируем на под-документы → `payload.documents[]` (уже есть, F5). Вопросы:
1. `documents[]` в webhook достаточно, или нужен родитель-ребёнок в БД (`jobs.parent_job_id`)?
2. Семантика v1-поля `payload.extracted` для файлов, СТАВШИХ multi-doc, меняется: было «весь документ», станет «доминирующий сегмент». Для v1-only консьюмера, читающего только `extracted` — поведенческая (не схемная) регрессия. Фиксируем «extracted = доминантный сегмент» + per-consumer флаг? (`TZ_CLASSIFIER_PACKET_V2.md` §7)
3. Нужен ли per-segment `needs_review`/`status` в `documents[]` (сейчас needs_review per-job — ошибочный/паспортный сегмент не уходит на ревью независимо)?

#### Answer
SLAI 2026-07-12: **GO по всем трём + 3 просьбы.**
1. **`documents[]` достаточно**, `parent_job_id` в нашей БД не требуется (SLAI уже
   создаёт N документов из `documents[]`, мультидок у них в бою).
2. **«extracted = доминантный сегмент» принимают, per-consumer флаг не нужен**
   (читают `documents[]`, не только `extracted`). Просьба: добавить в payload
   **`is_composite: true` + индекс доминанта** — чтобы было явно.
3. **Per-segment `needs_review`/`status` — ДА, нужен** (один спорный сегмент не
   должен тормозить весь файл) + **per-segment `confidence`** (полоска на карточке).
Две просьбы SLAI (важно для их пайплайна): (а) **эхо `metadata.order_hint` на
КАЖДЫЙ сегмент** композита (якорь «папка → заказ»); (б) **стабильный id сегмента**
(`job_id#index` или uuid) — для идемпотентности/дедупа при повторной доставке.

#### Resolution
Фаза 2: `ExtractedDocumentEntry` расширяется — `status`/`needs_review` +
`confidence` per-segment, `segment_id` = `job_id#index`, эхо `order_hint`. Payload
получает `is_composite: true` + `dominant_index`. `secondary_role` (двойные доки) =
каталожный slug. Правки аддитивны → **minor-бамп `schema_version`** (Q17).
...
| 2026-07-15 | **Deep-pass (второй ярус для нераспознанного остатка) + сверка правил загрузки из папок — обе ветки со SLAI закрыты.** Заведены и сразу RESOLVED **Q-DEEPPASS-1** (широкая категория+резюме+вердикт для unknown/refusal; возврат в конвейер при mapped; `_deep` у всех; `schema_version` 1.3→1.4; `cargo_photo` по предложению SLAI; коммиты `f9a909c`/`61dfbfc`/`5699bd2`; гейт `DEEP_PASS_ENABLED` OFF — ждёт включения на боксе) и **Q-INGEST-1** (5/5 правил передачи из папок совпали, разночтений нет: `order_hint` на каждом файле вкл. вложенные подпапки, папка=заказ 1:1, отдельные файлы vs композит, SHA-дедуп для почасовой перезаливки, multipart-канал). Попутно (не SLAI): tesseract timeout+SIGKILL против зависаний растровых сканов (`d0a5399`), детерминированные ошибки без 3× ретраев. Переписка: `SLAI_DEEP_PASS_BRIEF` / `SLAI_INGEST_RULES_SYNC` / ответы `SLAI_ANSWERS_2026-07-15*`. |

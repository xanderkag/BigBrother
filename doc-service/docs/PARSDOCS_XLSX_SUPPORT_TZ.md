...
> 🟡 **ЧАСТИЧНО.** Парсер есть (`src/pipeline/ocr/xlsx.ts`), но сквозной приём
> `.xls/.xlsx` в пайплайне ещё нестабилен (pre-processing Tier-3, формат-гэп —
> см. `STATUS.md`). Это ТЗ — исходная спецификация; живой статус сверять по
> `STATUS.md` / `SLAI_INTEGRATION_BACKLOG.md`.
...
Добавляется `XlsxEngine` рядом с существующими (PdfTextEngine, TesseractEngine, VisionLlmEngine, YandexVisionEngine). **Контракт (реализован — детали в `src/pipeline/ocr/xlsx.ts`):**

- `supportsMimeType`: 3 MIME — `application/vnd.ms-excel` (.xls),
  `…spreadsheetml.sheet` (.xlsx), `…ms-excel.sheet.macroEnabled.12` (.xlsm)
- `readFile` с `cellDates:true` (даты → `Date`), `cellFormula:false` (значения, не формулы), `cellNF:false`
- каждый лист → CSV (`sheet_to_csv`, `blankrows:false`, `strip:true`), секции `=== Sheet: <name> ===`
- скрытые листы (`!hidden`) — skip; лист **> 50 000 ячеек** → header + `[SKIPPED: N cells > 50k limit]`
- `confidence: 1.0` (точное чтение, не вероятностное), metadata `{ sheets, total_cells }`
...
- [`../../docs/FILE_INTAKE_OVERVIEW.md`](../../docs/FILE_INTAKE_OVERVIEW.md) — общий обзор приёма входящих файлов (XLSX — один из форматов)

...
async function processJobInner(
  ...
  try {
    ...
    //   2. КАРТИНКА/скан с типом от ТЕКСТ-классификатора (любая уверенность) —
    //      vision смотрит на изображение и решает: реальный тип → извлекаем;
    //      фото/не-документ → сброс ложного типа, извлечение отменяем (на фото
    //      полей нет). Подробности порога — в комментарии у imageUncertain.
    ...
    // Для картинок vision-проверка типа ОБЯЗАТЕЛЬНА (не только при низкой
    // уверенности): keyword и текст-LLM читают один и тот же OCR-текст надписей
    // с коробки и могут УВЕРЕННО согласиться на ложном типе (出货图片.pdf: фото
    // коробок → cert_of_origin с conf=1.0 от обоих). Согласие двух текстовых
    // источников о картинке ничего не доказывает — доказывает только взгляд на
    // само изображение. Исключения: hint (тип задан потребителем) и vlm (тип
    // уже получен по изображению в §P2-2).

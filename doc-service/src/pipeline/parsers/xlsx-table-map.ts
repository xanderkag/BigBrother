...
/**
 * Разметка колонок: какое поле схемы в какой колонке.
 *
 * Номера СТРОК сюда не входят намеренно — их знает анализатор, а модель нет
 * (в промпте номеров строк листа не показывают). См. `applyColumnMapping`.
 */
...
export function applyColumnMapping(
  ...
  // Границы данных задаёт КОД, а не модель.
  //
  // Раньше здесь доверялись присланному `header_row`. Это было ошибкой уровня
  // системы координат: в промпте у модели вообще нет номеров строк листа —
  // она видит перечень областей с шапками и парой примеров. Её «header_row: 0»
  // означало «первая строка показанной области», а код читал это как «строка 0
  // ЛИСТА» и начинал раскладку с самого верха. На боевом инвойсе в артикул
  // поехала шапка бланка: «Morion Ltd.», «Ul. Nekrasova, 44, liter A…»
  // (заполненность name упала до 0.53, путь отбраковался).
  //
  // Модель отвечает за смысл (какая область — товары, какая колонка — цена),
  // код — за строки. Начало и конец данных берём у анализатора.
  const start = cand.dataStartIndex;
...
export async function chooseRegionsAndMapColumns(
  ...
  const prompt =
    ...
    'другой язык, выдержка или дубль того же списка) — верни только ОСНОВНУЮ, самую ПОЛНУЮ ' +
    '(больше строк данных), иначе позиции задвоятся или потеряются. ' +
    'Если же позиции документа разбиты по нескольким областям (продолжение списка) — верни все. ' +
    `Доступные поля: ${fields.join(', ')}. ` +
    'Верни regions — массив объектов {region, mapping}, где region — номер области, ' +
    'mapping — массив пар {field, column} (column — номер колонки в квадратных скобках). ' +
    'Номера строк указывать не нужно — границы таблицы известны. ' +
    'Включай ТОЛЬКО поля, которые реально есть; не выдумывай отсутствующие. ' +
    'Ориентируйся на СМЫСЛ заголовков, а размер учитывай только при выборе между дублями.';
  ...
  for (const entry of list) {
    ...
    seenRegions.add(idx);
    out.push({ region, mapping: { columns } });
  }

...
export function findDocNumber(text: string, kindWords: string): string | undefined {
  // e.g. "Счёт № 123 от ...", "УПД № 456 от ...", "СЧЁТ No INV-2025-104"
  //
  // Два подводных камня в реальных PDF (фрахт-счета SLAI, 2026-05-20):
  //   1. pdf-parse декомпозирует «№» (U+2116) в ASCII «No». Поэтому numero-
  //      токен — альтернация (?:№|no|n°|nº|#), а не один символ «№».
  //   2. Без этого regex ловил «No» / «на» (из «Счёт на оплату») как номер.
  //      Лечим тем, что (а) опционально съедаем «на оплату» и numero-токен
  //      ДО захвата, и (б) требуем хотя бы одну цифру в самом номере —
  //      «No»/«на» цифр не содержат и больше не матчатся.
  const re = new RegExp(
    `(?:${kindWords})\\s*(?:на\\s+оплату\\s*)?(?:№|no|n[°º]|#)?\\s*` +
      `([A-ZА-Яa-zа-я0-9\\-\\/]*\\d[A-ZА-Яa-zа-я0-9\\-\\/]*)`,
    'i',
  );

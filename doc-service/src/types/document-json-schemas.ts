...
// SLAI 2026-07-10: per-container вес/объём/места. Раньше containers[] нёс
// только number → SLAI не мог собрать пер-контейнерную сверку
// «Σ по контейнерам = итог заказа = итог ГТД». Добавлены ОПЦИОНАЛЬНЫЕ поля:
// заполняются ТОЛЬКО когда в документе есть табличка «по контейнерам»
// (обычно пакинг-лист или контейнерная разбивка в коносаменте). Если такой
// таблички нет — модель кладёт один number, как раньше (back-compat).
const CONTAINERS = {
  type: 'array',
  description:
    'Контейнеры (ISO 6346: 4 буквы + 7 цифр, напр. MSCU1234567). ' +
    ...
    'По одному объекту на контейнер. ЕСЛИ в документе есть разбивка веса/объёма/' +
    'мест ПО КОНТЕЙНЕРАМ (табличка «контейнер → вес/объём/места», часто в ' +
    'пакинг-листе или коносаменте) — заполни gross_weight_kg / net_weight_kg / ' +
    'volume_m3 / packages для каждого. Нет разбивки — оставь только number. ' +
    'Если контейнеров нет — опусти поле.',
  items: {
    type: 'object',
    properties: {
      number: { type: 'string', description: 'Номер контейнера, формат ISO 6346 (напр. MSCU1234567)' },
      gross_weight_kg: { type: 'number', description: 'Вес брутто по этому контейнеру, кг (если в документе есть разбивка)' },
      net_weight_kg: { type: 'number', description: 'Вес нетто по этому контейнеру, кг (если есть)' },
      volume_m3: { type: 'number', description: 'Объём груза в этом контейнере, м³ (если есть)' },
      packages: { type: 'number', description: 'Число мест/упаковок в этом контейнере (если есть)' },
    },
  },
} as const;

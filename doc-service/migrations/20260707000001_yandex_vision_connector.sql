...
-- (connector='yandex_vision', unit_kind='pages') — и при успехе, и при падении
-- движка на N-й странице (первые N-1 уже оплачены).
--
-- ⚠ СУТОЧНЫЙ ЛИМИТ — МЯГКИЙ, не резервирующий. checkConsumerQuota проверяется
-- ОДИН РАЗ на старте джобы (`used < cap`), а расход списывается после прогона
-- документа. Поэтому лимит можно перерасходовать примерно на
-- (страниц в документе × параллельных воркеров): при used=99, cap=100
-- 300-страничный PDF уйдёт целиком. Это потолок «мягко тормозить», а не
-- жёсткая отсечка. Месячный cap отображается, но не энфорсится вовсе —
-- существующее поведение всех коннекторов, не регрессия этой миграции.
--
...
-- Down Migration
BEGIN;
-- gateway_consumer_budgets.connector — FK на gateway_connectors(slug) БЕЗ
-- ON DELETE CASCADE. Если админ успел задать бюджет потребителя по этому
-- коннектору, удаление строки коннектора падает на foreign key violation.
-- Сносим зависимые бюджеты первыми.
DELETE FROM gateway_consumer_budgets WHERE connector = 'yandex_vision';

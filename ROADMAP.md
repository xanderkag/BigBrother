...
00. **ВНУТРЕННИЙ БИЛЛИНГ + UI — приоритет. ТЗ: [`BILLING_INTERNAL_TZ.md`](doc-service/docs/BILLING_INTERNAL_TZ.md) · UI-аудит: [`UI_AUDIT_2026-07-23.md`](doc-service/docs/UI_AUDIT_2026-07-23.md)** (BILL-1 ✅ задеплоен, BILL-2…6 ждут). Реализацию добивает отдельный агент — макеты «Расходы»/«Бюджеты по инстансам» согласовать перед BILL-6.
    ...
    **Почему сейчас** (сверено по коду): `computeJobCost` считает per-job и пишет `jobs.cost_rub`, но **агрегации нет ни одной** — на «почему 600 ₽» отвечали SQL руками; бюджетов нет; `llm_gateway_usage` пишет токены, но не деньги (второй канал расхода невидим); `config.cost` зашит под Yandex и врёт, как только провайдеров в инстансе больше одного — а их уже больше (Yandex + локальный vLLM + Anthropic).
    ...
    |---|---|---|
    | **BILL-1** ✅ Модель себестоимости (`7836455`, задеплоен asha) | `provider_settings.rates` + `jobs.cost_breakdown` со снимком; миграция `20260723000001`; ставки засеяны + **yandex-ai-studio дозасеян вручную** (в миграции пропущен) | сделано; живой прогон покажет `cost_breakdown` на первой новой джобе |
    ...
    **СОСТОЯНИЕ 2026-07-23 (для агента, который добивает):**
    - **BILL-1 ✅** задеплоен на asha; ставки у всех активных провайдеров есть.
    - **Tenant split ✅** (запрос SLAI 2026-07-23): заведены орги `slai-bctt` (webhook→bctt.sls24.ru) и `slai-app` (→app.sls24.ru) через `provision-instance`; все 4 стенда развязаны (было — все под общим токеном → писали в negabarit). Биллинг разбора теперь различим по `organization_id`. **Хвост:** raw-токены отдать SLAI защищённым каналом (лежат на столе владельца), удалить после.
    - **BILL-4 расширен** (`78299b1`): все стенды ходят в чат-шлюз за ассистентом → нужен разрез `llm_gateway_usage.caller` по инстансу (свой gateway-ключ на стенд), сейчас всё под общим `parsdocs`.
    - **UI — [`UI_AUDIT_2026-07-23.md`](doc-service/docs/UI_AUDIT_2026-07-23.md).** Реализуемо СЕЙЧАС (бэкенд готов): (1) редактор ставок провайдера в `Providers.tsx` (`rates` сейчас правятся только SQL); (2) `cost_breakdown` в `JobDetail.tsx`. Ждут бэкенда: бюджет по инстансу (BILL-5) → форма из макета; экран «Расходы» (BILL-6, нужен BILL-2 API); обзор тенантов в `Organizations`. Есть уже: бюджет шлюза в `Integrations`, `ProvisionInstanceModal`, колонка ₽/док.

...
| **EXT-CLASS-1** — `special_permit` (Росавтодор) + `booking_request` (по §c решению SLAI) + waybill classifier тюнинг по их PDF | После WW-23 стабилизации | 1.5-2.5 дня. SLAI запрос 2026-06-XX (Q-CLASS-MATRIX). `special_permit` базовая schema (permit_no/issued_by/valid_to/route/vehicle/oversized_dimensions) переиспользует существующие `transport.permit.*` поля. `booking_request` — клон `transport_request` с `requestor.kind='forwarder'`. |

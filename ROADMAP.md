...
00. **ВНУТРЕННИЙ БИЛЛИНГ — 6 эпиков. ТЗ: [`doc-service/docs/BILLING_INTERNAL_TZ.md`](doc-service/docs/BILLING_INTERNAL_TZ.md)** (2026-07-22, ничего не реализовано).

    **Решения владельца:** внутренний учёт — счетов, наценки и тарифных планов наружу НЕ делаем · бюджет **предупреждает**, не блокирует · ставки переносятся с инстанса **на провайдера**.

    **Почему сейчас** (сверено по коду): `computeJobCost` считает per-job и пишет `jobs.cost_rub`, но **агрегации нет ни одной** — на «почему 600 ₽» отвечали SQL руками; бюджетов нет; `llm_gateway_usage` пишет токены, но не деньги (второй канал расхода невидим); `config.cost` зашит под Yandex и врёт, как только провайдеров в инстансе больше одного — а их уже больше (Yandex + локальный vLLM + Anthropic).

    | Эпик | Что | Приёмка (кратко) |
    |---|---|---|
    | **BILL-1** Модель себестоимости | `provider_settings.rates` + `jobs.cost_breakdown` со **снимком** применённой ставки; `config.cost` → fallback; заполнить ставки | смена ставки НЕ меняет уже посчитанные задачи; нет ставки → `estimate`, а не ноль |
    | **BILL-2** Отчёты | `GET /billing/summary` (разрезы org / project / doc_type / provider / day, фильтр `cost_basis`) + `/budgets` + `/job/:id` | сводка за июль воспроизводит цифры, считанные тогда руками |
    | **BILL-3** Экономия от кэша | `would_cost` на кэш-хите, строка «сэкономлено» | повтор файла → `cost_rub=0`, `would_cost` = стоимость оригинала |
    | **BILL-4** Стоимость шлюза | `llm_gateway_usage` += деньги, ставка по алиасу (учесть per-alias upstream `735feee`) | расход шлюза виден отдельно от расхода разбора |
    | **BILL-5** Бюджеты | `budgets` + `budget_events` + sweeper; `hard_limit_rub` зарезервирован под будущую блокировку | ровно одно событие на порог за период; обработка НЕ останавливается |
    | **BILL-6** UI «Расходы» | **начинается с макета и согласования, не с кода** | цифры на экране сходятся с `/billing/summary` |

    **Порядок жёсткий** — каждый следующий эпик опирается на данные предыдущего; BILL-1 фундамент.

    **Осознанные ограничения:** историю не пересчитываем (старые задачи не знают провайдера) — точка отсечения по дате миграции. Открыто: валюта USD (Anthropic — считать раздельно по валютам, единый ₽-итог требует курса и политики его фиксации) и переводная цена локального железа (числа нет → локальные задачи идут как `estimate`).
...
- ✅ **Стоимость ₽/док в списке разобранных документов (2026-07-13, `beac1f6`)** — owner-запрос (триггер 600₽ на eval-БКТ). Сделано целиком: `config.cost` (ставки Yandex Vision+AI Studio, env-переопр.) + `pipeline/cost.ts` (LLM-токены + OCR-страницы × ставки, только yandex; estimate «≥» когда неполно) + миграция `ocr_pages` + finalize/toApi (`cost_rub`+`cost_estimate`) + api-schema + колонка «Стоимость» в UI-списке. 6 cost-тестов, бэкенд-сьют 1429 зелёных. **На asha:** применить миграцию + подтвердить/подкрутить ставки в config (деплой — не агент).
...
- **⬜ VL-FINETUNE-PILOT — ЭПИК (до востребования): пилот дообучения Qwen3-VL-8B на наших сканах.** ТЗ от DevOps/GPU: `Desktop/ТЗ_Дообучение-VL-пилот_2026-07-21.md`; наш план и факт-ответы на §3 — в [`TECH_DEBT.md`](./TECH_DEBT.md) → VL-FINETUNE-PILOT. Кратко: пилотный тип — `commercial_invoice` (132 скана, 83% needs_review — единственный, где сходятся боль+история); главный блокер — **валидированных меток почти нет** (1 правка оператора на всю базу), сначала строится «фабрика меток» через существующий review-flow. Триггер запуска — команда владельца.

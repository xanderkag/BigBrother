...
| **EXT-CLASS-1** — `special_permit` (Росавтодор, **полноценная extraction-схема** не только классификация) + `booking_request` (по §c решению SLAI) + waybill classifier тюнинг по их PDF | После WW-23 стабилизации | 2-2.5 дня. SLAI 2026-06-XX (Q-CLASS-MATRIX + Q-PERMIT-1 уточнение). **`special_permit` поля P0** (для первого негабарит-клиента): permit_number, permit_valid_from/until, issuing_authority, vehicle.{plate, trailer_plate}, route.{from, to, waypoints[]}, cargo.{length_cm, width_cm, height_cm, total_weight_kg, axle_loads_kg[], axles_count}, escort.{required, type}, restrictions. Большая часть переиспользует существующие `transport.permit.*` / `transport.cargo.*` / `transport.escort.*` поля; новое — `waypoints[]`, `axle_loads_kg[]`, `restrictions`. Vision-llm path обязателен (сканы/печати/штампы Росавтодора). `booking_request` — клон `transport_request` с `requestor.kind='forwarder'`. |

...
00. **Внутренний биллинг — ТЗ `doc-service/docs/BILLING_INTERNAL_TZ.md` (2026-07-22, не реализовано).** Решения владельца: внутренний учёт (счетов наружу НЕ делаем) · бюджет предупреждает, не блокирует · **ставки переносятся с инстанса на провайдера**. Причина: `cost_rub` считается per-job, но агрегации нет ни одной (на «почему 600₽» отвечали SQL руками), бюджетов нет, расход через LLM-шлюз в деньгах не считается вообще, а глобальные ставки врут при нескольких провайдерах в инстансе (Yandex + локальный vLLM + Anthropic). Порядок этапов: (1) `provider_settings.rates` + `jobs.cost_breakdown` со снимком применённой ставки → (2) заполнить ставки, `config.cost` в fallback → (3) `GET /billing/summary` с разрезами → (4) экономия от кэша (`would_cost`) → (5) стоимость в `llm_gateway_usage` → (6) `budgets`+`budget_events`+sweeper → (7) UI «Расходы». Историю не пересчитываем (старые задачи не знают провайдера) — точка отсечения по дате миграции. Открыто: валюта USD (Anthropic) и переводная цена локального железа.

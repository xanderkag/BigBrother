...
0. **Классификатор v2 — пакеты БКТ (ТЗ `TZ_CLASSIFIER_PACKET_V2.md`, golden `BCTT_GROUNDTRUTH.md`, github/main `3448e74`).** Ручной разбор 51 дока показал: ~35/51 — композиты (стопка 3–15 доков в одном скане), пайплайн вешает один тип на всю стопку — главный провал. Реализующий агент ведёт по фазам:
   ...
   - **Статус реализации:** Фаза 1 ✅ (`b3e0018`). Фаза 2 почти готова: детектор границ ✅ (`dcbad8e`), splitter-интеграция ✅ (`ea23a84`), контракт-поля SLAI ✅ (`18ac600`, schema 1.3), §8.1 raw_text-маска ✅ (`5a5c23c`), §8.5b не-в-LLM ✅ (`5ef0fd2`). **🔓 ВСЕ ПДн-блокеры §8.1–8.5 закрыты — композит-с-паспортом разблокирован** (1373 теста зелёных). Осталось Фаза 2: config `segment*`/`ID_EXTRACT_ENABLED` (дефолты работают), LLM-адаптер per-page P0-1 (границы уже компенсируют), P0-0 guard. Дальше Фаза 3 (P2) + eval-bctt harness на 51 файл.

...
0. **Классификатор v2 — пакеты БКТ (ТЗ `TZ_CLASSIFIER_PACKET_V2.md`, golden `BCTT_GROUNDTRUTH.md`, github/main `3448e74`).** Ручной разбор 51 дока показал: ~35/51 — композиты (стопка 3–15 доков в одном скане), пайплайн вешает один тип на всю стопку — главный провал. Реализующий агент ведёт по фазам:
   ...
   - **⬜ Следующий шаг — прогон на asha:** задеплоить, прогнать `eval-bctt` на корпусе БКТ (51 файл, `/root/bctt-docs`), получить M1/M2/M4. По результатам решить, нужны ли отложенные улучшения.
   - **Отложено (runtime-зависимо, катить вслепую рискованно; границы компенсируют):** P0-1 LLM-адаптер per-page + P0-0 OCR-форсинг (Фаза 2); P2-2 VLM-classify плохих фото + P2-3 `CLASSIFY_PROVIDER_ID` A/B (Фаза 3). Валидировать по данным eval-bctt.

...
| **EXT-CLASS-1** — `special_permit` (Росавтодор, **полноценная extraction-схема** не только классификация) + `booking_request` (по §c решению SLAI) + waybill classifier тюнинг по их PDF | После WW-23 стабилизации, spec frozen 2026-06-XX | 2-2.5 дня. SLAI 2026-06-XX (Q-CLASS-MATRIX + Q-PERMIT-1 spec frozen). **`special_permit` поля P0**: permit_number, permit_valid_from/until, issuing_authority, vehicle.{plate, trailer_plate}, route.{from, to, waypoints[]}, cargo.{length_cm/width_cm/height_cm + дубликат метров, total_weight_kg, axle_loads_kg[], axles_count}, escort.{required, type ENUM `gibdd\|cover-vehicle\|pilot-driver\|none` kebab-case 1:1 SLAI}, restrictions. **CM обязательны** (их код-путь `CargoUnit.lengthCm/widthCm/heightCm`). P0-min для матчера: permit_number + permit_valid_until + vehicle.plate. Vision-llm path обязателен. `_normalized_fields[permit.number, vehicle.plate]` для их новой `matcher.phase='permit'`. `booking_request` — клон `transport_request` с `requestor.kind='forwarder'`. |
| **EXT-CLASS-2** — `AWB` (Air Waybill) + `manifest` (cargo manifest) + `phytosanitary_certificate` + `veterinary_certificate` + тюнинг `commercial_invoice` под ВЭД (incoterms/hs_code/country_of_origin/customs_value) | EXT-CLASS-1 | ~3 дня. P1 SLAI matrix. AWB: 11-digit IATA awb_number / airline / origin-destination airport / flight_no / pieces / gross_weight / charges. Manifest: voyage/flight + list of items per BL/AWB. |
| **EXT-CLASS-3** — `CIM` (международная ЖД конвенция) + `СМГС` (CIS+China ЖД) | EXT-CLASS-2 | ~1.5 дня. Две отдельные схемы (consignor/consignee, station_of_dispatch/destination, wagon_number, container_no, cargo, route_via). |
| **EXT-LLM-GATEWAY-DADATA** — passthrough `POST /v1/dadata/findById/party` (+опц. `/suggest/party`) к suggestions.dadata.ru с нашим DADATA_API_KEY (из env или provider_settings kind='dadata'). По аналогии с /v1/embeddings: тонкий passthrough, native DaData-shape в обе стороны, usage-log per-PAT. Делать в пакете с активацией LLM-gateway после WW-23 демо. | После cutover SLAI на наш gateway | ~1-2 часа кода. SLAI 2026-06-XX (Q-DADATA-1). DaData geo-доступен из РФ — никакого outbound-прокси не надо (в отличие от Anthropic/OpenAI). Auth `Authorization: Token <key>` (не Bearer). Цель cutover: SLAI выкидывает DADATA_API_KEY у себя, ключ держим мы. |
